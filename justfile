...
# Download articles
download *ARGS:
    UV_CACHE_DIR=.uv-cache uv run -m econprior.cli download {{ARGS}}

# Fill missing abstracts from other sources
fill *ARGS:
    UV_CACHE_DIR=.uv-cache uv run -m econprior.cli fill {{ARGS}}
...
stats *ARGS:
    UV_CACHE_DIR=.uv-cache uv run -m econprior.cli stats {{ARGS}}
...
stats-sync:
    ...
    UV_CACHE_DIR=.uv-cache uv run -m jupyter nbconvert --to notebook --execute --inplace notebooks/show_stats.ipynb
